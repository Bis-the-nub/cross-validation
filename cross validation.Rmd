...
cv_df = 
  ...
  mutate(
    ...
    smooth_mod  = map(train, \(df) mgcv::gam(logratio ~ s(range), data = df)),
    wiggly_mod  = map(train, \(df) mgcv::gam(logratio ~ s(range, k = 50), sp = 10e-8, data = df))) |> 
  mutate(
...
cv_df = 
  ...
  mutate(
    linear_mod  = map(train, \(df) lm(armc ~ weight, data = df)),
    smooth_mod  = map(train, \(df) mgcv::gam(armc ~ weight + weight_cp7, data = df)),
    pwl_mod  = map(train, \(df) mgcv::gam(armc ~ s(weight), data = df))) |> 
  mutate(
    rmse_linear = map2_dbl(linear_mod, test, rmse),
    rmse_smooth = map2_dbl(smooth_mod, test, rmse),
